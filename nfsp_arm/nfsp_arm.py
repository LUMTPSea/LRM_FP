...
class NFSP_ARM(rl_agent.AbstractAgent):
	...
	def __init__(self,
		...
		self._rl_agent = arm.ARM(device, player_id, state_representation_size,
		...
		self._avg_network = arm.MLP(state_representation_size,
										  self._layer_sizes, num_actions).to(self._device)

		self._savers = [
			("q_network", self._rl_agent._q_network),
			("v_network", self._rl_agent._v_network), 
	...
	def _learn(self):
		...
		info_states = torch.Tensor(
			[t.info_state for t in transitions]).to(self._device)
		action_probs = torch.Tensor(
			[t.action_probs for t in transitions]).to(self._device)
	...
	def restore(self, checkpoint_dir):
		...
		"""
		for name, model in self._savers:
			full_checkpoint_dir = self._full_checkpoint_name(
				checkpoint_dir, name)
